function [CM, DM] = simcolumn_connectivity_ThtoL4Conn(preCells, postCells)
...
for i = 1:length(PreType_Unique)
    switch PreType_Unique(i)  
        case(1) % presynaptic cell is spiny pyramidal
            CM(:, preIND(i)+1:preIND(i+1)) = simConn_ThtoL4_ThtoAll(AxonDendOverlap(:, preIND(i)+1:preIND(i+1)), ...
